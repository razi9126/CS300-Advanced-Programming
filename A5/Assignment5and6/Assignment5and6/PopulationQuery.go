//README
// All parts work, one small problem is that one of the values for the new x and y co-ordinates is greater than the given
// dimension(index out of bound). This is because i did all the calculations in float64 and simply converted to (int+1) at the end.
// However it's not too big of a problem. I even know the coordinates for which i encounter this error(13,4) ( it should have been 12,4 bucketssince I'm
// adding +1 to all my answers it goes to 12. Anyways, i simply ignore this value. Its on a boundary so it doesn't even affect the answers and the test cases.
// Also I calculate the total population by only adding valid values hence it doesn't affect the total pop too. All in all, the program works as required
// by the handout with the exception of just 1 value that I'm simply ignoring.
// Also, for the parallel part 2, i sometimes get the wrong answer if i check for 1 1 3 3 first(only sometimes). Though if icheck for 3 3 6 8 
// first, then i always get the right answers. I know its because i've not handled some channel correctly. Sorry :/
...
func makeGrid(cD []CensusGroup, xdv float64, ydv float64, minlt float64, minln float64, xdm int, ydm int) ([][] int){
	...
	if len(cD)<20000 {
		...
		//have to prallelize this summing
     	...
     	// arraySum(func_grid, func_grid2, 138, 276, xdm, ydm)

     	// for y:= 0; y<ydm; y++{
     	// 	for x:= 0; x<xdm; x++{
     	// 		func_grid[y][x] = func_grid2[y][x] + func_grid[y][x]
     	// 	}
     	// }
     	return func_grid
     }
