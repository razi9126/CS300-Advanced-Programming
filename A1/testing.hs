...
-- numberOfSpaces [] = 0.0
-- numberOfSpaces (x:xs) 
-- 	| x==" " = 1.0 + (numberOfSpaces xs)
-- 	| otherwise = 0.0 + (numberOfSpaces xs)

-- sumOfVal [] = 0
-- sumOfVal (x:xs) = x + sumOfVal xs

-- z (x:xs)= realToFrac (var (numbSp (x:xs) [] 0))
...
-- rmSmall n (x:xs)	| (length x) == n = x:rmSmall n (xs)
-- 					| otherwise		  =  rmSmall n (xs)
					
-- g= ["a","b","c","d"]
-- dd = insSp g
-- df = applyNTimes 2 bSp dd
-- dg = rmdups df
...
-- joinVal (a,b) text = (a ++ [text ++ "-"],b)

-- d = sum' (search' "creative" enHyp)


-- e=[(["A"], ["creative", "man"])] ++ map (otherValues (["A"], ["creative", "man"]) 9 enHyp) d

-- x=wordLengths(fst (["A"], ["creative", "man"]))

-- -- filter ((a,b),a==[], b == [])
-- otherValues (a,b) n m y 
-- 	| wordLengths (fst (joinNew (a,b) y)) - 1 > n = ([],[])
-- 	| otherwise = joinNew (a,b) y 
...
-- Takes the list of tuples and the cost function that is to be applied to the fst value of the tuple and retruns the tuple along with its cost
sendToLC [(a,b)] _ = (a,b)
...
justifyText func1 wordmap n dat = [unwords val] ++  justifyText func1 wordmap n (unwords (snd (bestLineBreak func1 wordmap n (words dat))) )
	where val = (fst (bestLineBreak func1 wordmap n (words dat)))
